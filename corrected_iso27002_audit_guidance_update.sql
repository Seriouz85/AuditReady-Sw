--
-- CORRECTED ISO 27002 Audit Ready Guidance Update Script
-- This script ONLY updates the audit_ready_guidance column for ISO 27002 requirements
-- Uses the ACTUAL database format (5.1, 5.2, etc.) not A.5.1 format
-- Based on actual database inspection using Supabase MCP
--
...
DO $$
DECLARE
  v_iso27002_standard_id UUID := '8508cfb0-3457-4226-b39a-851be52ef7ea';
  v_affected_count INTEGER;
BEGIN
  -- Validate the standard exists
  IF NOT EXISTS (SELECT 1 FROM standards_library WHERE id = v_iso27002_standard_id) THEN
  ...
  IF v_affected_count != 93 THEN
    RAISE EXCEPTION 'Expected 93 ISO 27002 requirements, found %. Aborting update.', v_affected_count;
  END IF;
END $$;

-- ============================================================================
-- AUDIT READY GUIDANCE UPDATES
-- Using actual database control_id format (5.1, 5.2, etc.)
-- ============================================================================

-- Update 5.x Organizational controls (37 controls)
...
WHERE control_id = '5.1' 
  AND standard_id = '8508cfb0-3457-4226-b39a-851be52ef7ea';
...
• Train personnel on documented procedures',
  updated_at = NOW()
WHERE control_id = '5.37' 
  AND standard_id = '8508cfb0-3457-4226-b39a-851be52ef7ea';

-- Update 6.x People controls (8 controls)
UPDATE requirements_library 
SET 
...
-- Update 7.x Physical and environmental security controls (14 controls)
UPDATE requirements_library 
SET 
...
• Maintain records of equipment disposal and data sanitization',
  updated_at = NOW()
WHERE control_id = '7.14' 
  AND standard_id = '8508cfb0-3457-4226-b39a-851be52ef7ea';

-- Update 8.x Technology controls (34 controls)
UPDATE requirements_library 
SET 
...
BEGIN
  ...
  WHERE standard_id = '8508cfb0-3457-4226-b39a-851be52ef7ea'
    AND audit_ready_guidance IS NOT NULL
    AND audit_ready_guidance != ''
