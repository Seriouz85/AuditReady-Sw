--
-- SAFE ISO 27002 Audit Ready Guidance Update Script
-- This script ONLY updates the audit_ready_guidance column for ISO 27002 requirements
-- It uses requirement_code matching for safety and includes comprehensive safeguards
--
...
-- 1. Create a backup table first (optional but recommended)
-- CREATE TABLE requirements_library_backup_$(date +%Y%m%d_%H%M%S) AS SELECT * FROM requirements_library WHERE standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

-- 2. Validate that we're only affecting ISO 27002 requirements
DO $$
DECLARE
  v_iso27002_standard_id UUID;
  v_affected_count INTEGER;
BEGIN
  -- Find ISO 27002 standard ID
  SELECT id INTO v_iso27002_standard_id 
  FROM standards_library 
  WHERE (name ILIKE '%ISO%27002%' OR name ILIKE '%27002%' OR code = 'ISO_27002')
  LIMIT 1;
  
  IF v_iso27002_standard_id IS NULL THEN
    RAISE EXCEPTION 'ISO 27002 standard not found. Aborting update.';
  END IF;
  
  -- Count requirements that would be affected
  SELECT COUNT(*) INTO v_affected_count
  FROM requirements_library
  WHERE standard_id = v_iso27002_standard_id
    AND requirement_code ~ '^A\.[5-8]\.\d+$'; -- Only A.5.x, A.6.x, A.7.x, A.8.x format
  
  RAISE NOTICE 'ISO 27002 Standard ID: %', v_iso27002_standard_id;
  RAISE NOTICE 'Requirements to be updated: %', v_affected_count;
  
  IF v_affected_count = 0 THEN
    RAISE EXCEPTION 'No ISO 27002 requirements found. Aborting update.';
  END IF;
  
  IF v_affected_count > 100 THEN
    RAISE EXCEPTION 'Too many requirements found (%). This may not be ISO 27002. Aborting update.', v_affected_count;
  END IF;
END $$;

-- ============================================================================
-- AUDIT READY GUIDANCE UPDATES
-- ============================================================================

-- Update A.5 Organizational controls (37 controls)
...
• Review policies regularly and update when significant changes occur',
  updated_at = NOW()
WHERE requirement_code = 'A.5.1' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');
...
• Regular review and update of role assignments',
  updated_at = NOW()
WHERE requirement_code = 'A.5.2' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Reduce opportunities for unauthorized or unintentional modification or misuse of assets.
**Implementation**:
...
• Regular review of duty assignments and access rights',
  updated_at = NOW()
WHERE requirement_code = 'A.5.3' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Ensure management requires all personnel to apply information security according to established policies.
**Implementation**:
...
• Regular communication of security expectations to all staff',
  updated_at = NOW()
WHERE requirement_code = 'A.5.4' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Maintain appropriate contacts with relevant authorities.
**Implementation**:
...
• Regular review and update of contact information',
  updated_at = NOW()
WHERE requirement_code = 'A.5.5' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Maintain contacts with special interest groups and security forums.
**Implementation**:
...
• Regular attendance at security conferences and events',
  updated_at = NOW()
WHERE requirement_code = 'A.5.6' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Collect and analyze information about information security threats to produce threat intelligence.
**Implementation**:
...
• Share threat intelligence with relevant stakeholders',
  updated_at = NOW()
WHERE requirement_code = 'A.5.7' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Ensure information security is integrated into project management.
**Implementation**:
...
• Review and approve security aspects before project closure',
  updated_at = NOW()
WHERE requirement_code = 'A.5.8' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Develop and maintain an inventory of information and other associated assets.
**Implementation**:
...
• Regular review and update of asset inventory',
  updated_at = NOW()
WHERE requirement_code = 'A.5.9' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Identify and document rules for acceptable use of information and assets.
**Implementation**:
...
• Regular review and update of acceptable use policies',
  updated_at = NOW()
WHERE requirement_code = 'A.5.10' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Ensure all organizational assets are returned upon termination of employment or contract.
**Implementation**:
...
• Include asset return requirements in employment contracts',
  updated_at = NOW()
WHERE requirement_code = 'A.5.11' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Classify information according to security needs based on confidentiality, integrity, and availability.
**Implementation**:
...
• Regular review of classification assignments',
  updated_at = NOW()
WHERE requirement_code = 'A.5.12' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Develop and implement procedures for information labelling in accordance with classification scheme.
**Implementation**:
...
• Monitor compliance with labelling procedures',
  updated_at = NOW()
WHERE requirement_code = 'A.5.13' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish rules and procedures for secure information transfer.
**Implementation**:
...
• Monitor and log information transfers',
  updated_at = NOW()
WHERE requirement_code = 'A.5.14' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Control physical and logical access to information and assets based on business requirements.
**Implementation**:
...
• Monitor and log access to sensitive assets',
  updated_at = NOW()
WHERE requirement_code = 'A.5.15' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Manage the full lifecycle of identities from creation to deletion.
**Implementation**:
...
• Integrate with HR systems for automated lifecycle management',
  updated_at = NOW()
WHERE requirement_code = 'A.5.16' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Control allocation and management of authentication information through management processes.
**Implementation**:
...
• Regular review and rotation of authentication credentials',
  updated_at = NOW()
WHERE requirement_code = 'A.5.17' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Provision, review, modify and remove access rights according to access control policy.
**Implementation**:
...
• Document all access right changes and approvals',
  updated_at = NOW()
WHERE requirement_code = 'A.5.18' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Manage information security risks associated with supplier products and services.
**Implementation**:
...
• Establish incident response procedures for supplier-related incidents',
  updated_at = NOW()
WHERE requirement_code = 'A.5.19' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish information security requirements in agreements with suppliers who handle organizational information.
**Implementation**:
...
• Define incident notification and response requirements',
  updated_at = NOW()
WHERE requirement_code = 'A.5.20' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Manage information security risks in the ICT products and services supply chain.
**Implementation**:
...
• Establish procedures for supply chain incident response',
  updated_at = NOW()
WHERE requirement_code = 'A.5.21' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Regularly monitor, review and audit supplier service delivery.
**Implementation**:
...
• Track supplier performance against security requirements',
  updated_at = NOW()
WHERE requirement_code = 'A.5.22' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish processes for acquisition, use, management and exit from cloud services.
**Implementation**:
...
• Plan for secure cloud service exit strategies',
  updated_at = NOW()
WHERE requirement_code = 'A.5.23' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Plan and prepare for managing information security incidents.
**Implementation**:
...
• Test incident response procedures regularly',
  updated_at = NOW()
WHERE requirement_code = 'A.5.24' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Assess information security events and decide if they should be categorized as incidents.
**Implementation**:
...
• Document decisions on event categorization',
  updated_at = NOW()
WHERE requirement_code = 'A.5.25' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Respond to information security incidents according to documented procedures.
**Implementation**:
...
• Conduct post-incident reviews and lessons learned',
  updated_at = NOW()
WHERE requirement_code = 'A.5.26' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Use knowledge from incidents to strengthen and improve information security controls.
**Implementation**:
...
• Share knowledge across the organization',
  updated_at = NOW()
WHERE requirement_code = 'A.5.27' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish procedures for identification, collection, acquisition and preservation of evidence.
**Implementation**:
...
• Ensure legal admissibility of collected evidence',
  updated_at = NOW()
WHERE requirement_code = 'A.5.28' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Plan how to maintain information security at appropriate level during disruption.
**Implementation**:
...
• Train staff on security procedures during disruptions',
  updated_at = NOW()
WHERE requirement_code = 'A.5.29' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Plan, implement, maintain and test ICT readiness based on business continuity objectives.
**Implementation**:
...
• Maintain documentation of ICT continuity arrangements',
  updated_at = NOW()
WHERE requirement_code = 'A.5.30' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Identify and manage legal, statutory, regulatory and contractual requirements for information security.
**Implementation**:
...
• Regularly review and update compliance requirements',
  updated_at = NOW()
WHERE requirement_code = 'A.5.31' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement appropriate procedures to protect intellectual property rights.
**Implementation**:
...
• Monitor and enforce intellectual property rights',
  updated_at = NOW()
WHERE requirement_code = 'A.5.32' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect records from loss, destruction, falsification, unauthorized access and release.
**Implementation**:
...
• Monitor access to and handling of records',
  updated_at = NOW()
WHERE requirement_code = 'A.5.33' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Identify and meet requirements for privacy and protection of personally identifiable information.
**Implementation**:
...
• Conduct privacy impact assessments',
  updated_at = NOW()
WHERE requirement_code = 'A.5.34' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Review information security management approach and implementation independently.
**Implementation**:
...
• Document review findings and recommendations',
  updated_at = NOW()
WHERE requirement_code = 'A.5.35' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Regularly review compliance with information security policies, procedures and standards.
**Implementation**:
...
• Implement corrective actions for non-compliance',
  updated_at = NOW()
WHERE requirement_code = 'A.5.36' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Document operating procedures for information processing facilities and make them available to personnel.
**Implementation**:
...
• Train personnel on documented procedures',
  updated_at = NOW()
WHERE requirement_code = 'A.5.37' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

-- Update A.6 People controls (8 controls)
UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Conduct background verification checks proportional to business requirements and perceived risks.
**Implementation**:
...
• Regular review of screening procedures and requirements',
  updated_at = NOW()
WHERE requirement_code = 'A.6.1' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: State information security responsibilities in contractual agreements with personnel and contractors.
**Implementation**:
...
• Regular review and update of contractual terms',
  updated_at = NOW()
WHERE requirement_code = 'A.6.2' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Provide appropriate information security awareness, education and training to personnel.
**Implementation**:
...
• Measure effectiveness of training programs',
  updated_at = NOW()
WHERE requirement_code = 'A.6.3' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Formalize and communicate disciplinary process for information security breaches.
**Implementation**:
...
• Document disciplinary actions and lessons learned',
  updated_at = NOW()
WHERE requirement_code = 'A.6.4' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Define information security responsibilities that remain valid after termination or employment change.
**Implementation**:
...
• Monitor compliance with post-employment obligations',
  updated_at = NOW()
WHERE requirement_code = 'A.6.5' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement confidentiality agreements reflecting organizational needs for information protection.
**Implementation**:
...
• Monitor compliance with confidentiality obligations',
  updated_at = NOW()
WHERE requirement_code = 'A.6.6' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement security measures for remote working to protect information outside organizational premises.
**Implementation**:
...
• Monitor and manage remote working security',
  updated_at = NOW()
WHERE requirement_code = 'A.6.7' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Provide mechanism for personnel to report observed or suspected information security events.
**Implementation**:
...
• Ensure timely response to reported events',
  updated_at = NOW()
WHERE requirement_code = 'A.6.8' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

-- Update A.7 Physical and environmental security controls (14 controls)
UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Define and implement physical security perimeters for areas containing information and assets.
**Implementation**:
...
• Regular review of perimeter security effectiveness',
  updated_at = NOW()
WHERE requirement_code = 'A.7.1' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect areas containing information and assets with appropriate entry controls.
**Implementation**:
...
• Regular review of entry control effectiveness',
  updated_at = NOW()
WHERE requirement_code = 'A.7.2' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Design and implement protection against environmental threats including natural disasters and attacks.
**Implementation**:
...
• Develop response procedures for environmental incidents',
  updated_at = NOW()
WHERE requirement_code = 'A.7.3' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Design and implement procedures for working in secure areas.
**Implementation**:
...
• Train personnel on secure area procedures',
  updated_at = NOW()
WHERE requirement_code = 'A.7.4' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Design and implement protection against comprehensive physical and environmental threats.
**Implementation**:
...
• Monitor compliance with secure area procedures',
  updated_at = NOW()
WHERE requirement_code = 'A.7.6' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Define and enforce clear desk and clear screen rules for information protection.
**Implementation**:
...
• Monitor compliance with clear desk policies',
  updated_at = NOW()
WHERE requirement_code = 'A.7.7' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Ensure equipment is sited securely and protected from threats.
**Implementation**:
...
• Monitor equipment security and status',
  updated_at = NOW()
WHERE requirement_code = 'A.7.8' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect assets located outside organizational premises.
**Implementation**:
...
• Monitor security of off-premises assets',
  updated_at = NOW()
WHERE requirement_code = 'A.7.9' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Manage storage media according to organizational classification scheme.
**Implementation**:
...
• Monitor and track storage media throughout lifecycle',
  updated_at = NOW()
WHERE requirement_code = 'A.7.10' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect information processing facilities from power failures and utility disruptions.
**Implementation**:
...
• Monitor utility systems and backup power',
  updated_at = NOW()
WHERE requirement_code = 'A.7.11' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect cables carrying power, data or information services from threats.
**Implementation**:
...
• Establish procedures for cable installation and maintenance',
  updated_at = NOW()
WHERE requirement_code = 'A.7.12' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Maintain equipment correctly to ensure availability, integrity and confidentiality of information.
**Implementation**:
...
• Verify equipment security after maintenance',
  updated_at = NOW()
WHERE requirement_code = 'A.7.13' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Ensure secure disposal or reuse of equipment containing storage media.
**Implementation**:
...
• Maintain records of equipment disposal and data sanitization',
  updated_at = NOW()
WHERE requirement_code = 'A.7.14' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

-- Update A.8 Technology controls (34 controls)
UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Protect information stored on, processed by or accessible via user endpoint devices.
**Implementation**:
...
• Monitor endpoint security status and compliance',
  updated_at = NOW()
WHERE requirement_code = 'A.8.1' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Restrict and control allocation and use of privileged access rights.
**Implementation**:
...
• Regular review of privileged access assignments',
  updated_at = NOW()
WHERE requirement_code = 'A.8.2' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Restrict access to information and application functions according to access control policy.
**Implementation**:
...
• Regular review of access permissions',
  updated_at = NOW()
WHERE requirement_code = 'A.8.3' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Appropriately manage access to source code, development tools and software libraries.
**Implementation**:
...
• Establish procedures for development tool access',
  updated_at = NOW()
WHERE requirement_code = 'A.8.4' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement secure authentication technologies and procedures based on access restrictions.
**Implementation**:
...
• Monitor authentication activities and failures',
  updated_at = NOW()
WHERE requirement_code = 'A.8.5' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Monitor and adjust resource usage according to current and expected capacity requirements.
**Implementation**:
...
• Plan for capacity scaling and upgrades',
  updated_at = NOW()
WHERE requirement_code = 'A.8.6' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement malware protection supported by appropriate user awareness.
**Implementation**:
...
• Monitor and respond to malware incidents',
  updated_at = NOW()
WHERE requirement_code = 'A.8.7' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Obtain information about technical vulnerabilities and evaluate organizational exposure.
**Implementation**:
...
• Prioritize and remediate vulnerabilities based on risk',
  updated_at = NOW()
WHERE requirement_code = 'A.8.8' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish, document, implement, monitor and review configurations including security configurations.
**Implementation**:
...
• Regular review of configuration standards',
  updated_at = NOW()
WHERE requirement_code = 'A.8.9' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Delete information stored in systems and devices when no longer required.
**Implementation**:
...
• Monitor compliance with data deletion requirements',
  updated_at = NOW()
WHERE requirement_code = 'A.8.10' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Use data masking in accordance with access control policy and business requirements.
**Implementation**:
...
• Monitor and audit data masking activities',
  updated_at = NOW()
WHERE requirement_code = 'A.8.11' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Apply data leakage prevention measures to systems, networks and devices handling sensitive information.
**Implementation**:
...
• Respond to data leakage incidents promptly',
  updated_at = NOW()
WHERE requirement_code = 'A.8.12' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Maintain and regularly test backup copies of information, software and systems.
**Implementation**:
...
• Monitor backup operations and verify data integrity',
  updated_at = NOW()
WHERE requirement_code = 'A.8.13' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement sufficient redundancy in information processing facilities to meet availability requirements.
**Implementation**:
...
• Monitor system availability and performance',
  updated_at = NOW()
WHERE requirement_code = 'A.8.14' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Produce, store, protect and analyze logs of activities, exceptions, faults and relevant events.
**Implementation**:
...
• Analyze logs for security incidents and anomalies',
  updated_at = NOW()
WHERE requirement_code = 'A.8.15' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Monitor networks, systems and applications for anomalous behavior to evaluate potential security incidents.
**Implementation**:
...
• Investigate and respond to detected anomalies',
  updated_at = NOW()
WHERE requirement_code = 'A.8.16' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Synchronize clocks of information processing systems to approved time sources.
**Implementation**:
...
• Establish procedures for time source management',
  updated_at = NOW()
WHERE requirement_code = 'A.8.17' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Restrict and control use of utility programs capable of overriding system controls.
**Implementation**:
...
• Establish approval procedures for utility program use',
  updated_at = NOW()
WHERE requirement_code = 'A.8.18' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Implement procedures to control software installation on operational systems.
**Implementation**:
...
• Test software in controlled environments before deployment',
  updated_at = NOW()
WHERE requirement_code = 'A.8.19' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Manage and control networks and network devices to protect information in systems and applications.
**Implementation**:
...
• Regular review of network security configurations',
  updated_at = NOW()
WHERE requirement_code = 'A.8.20' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Identify security mechanisms and requirements for network services in service agreements.
**Implementation**:
...
• Regular review of network service security',
  updated_at = NOW()
WHERE requirement_code = 'A.8.21' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Segregate groups of information services, users and systems on networks.
**Implementation**:
...
• Provide user awareness on web security risks',
  updated_at = NOW()
WHERE requirement_code = 'A.8.23' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Define and implement rules for effective use of cryptography including key management.
**Implementation**:
...
• Monitor and audit cryptographic implementations',
  updated_at = NOW()
WHERE requirement_code = 'A.8.24' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish and apply rules for secure development of software and systems.
**Implementation**:
...
• Train developers on secure coding practices',
  updated_at = NOW()
WHERE requirement_code = 'A.8.25' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Identify, specify and approve information security requirements when developing or acquiring applications.
**Implementation**:
...
• Approve security requirements before implementation',
  updated_at = NOW()
WHERE requirement_code = 'A.8.26' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Establish principles for engineering secure systems and apply to development activities.
**Implementation**:
...
• Review and update principles regularly',
  updated_at = NOW()
WHERE requirement_code = 'A.8.27' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Apply secure coding principles to software development.
**Implementation**:
...
• Use static and dynamic code analysis tools',
  updated_at = NOW()
WHERE requirement_code = 'A.8.28' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Define and implement security testing processes in the development lifecycle.
**Implementation**:
...
• Validate security controls before production deployment',
  updated_at = NOW()
WHERE requirement_code = 'A.8.29' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Direct, monitor and review activities related to outsourced system development.
**Implementation**:
...
• Include security clauses in outsourcing contracts',
  updated_at = NOW()
WHERE requirement_code = 'A.8.30' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Separate development, testing and operational environments to reduce unauthorized access risks.
**Implementation**:
...
• Monitor activities across different environments',
  updated_at = NOW()
WHERE requirement_code = 'A.8.31' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Subject changes to information processing facilities and systems to change management procedures.
**Implementation**:
...
• Monitor and audit change implementation',
  updated_at = NOW()
WHERE requirement_code = 'A.8.32' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Appropriately select, protect and control test information.
**Implementation**:
...
• Securely dispose of test data after use',
  updated_at = NOW()
WHERE requirement_code = 'A.8.33' 
  AND standard_id IN (SELECT id FROM standards_library WHERE name ILIKE '%27002%');

UPDATE requirements_library 
SET 
  audit_ready_guidance = '**Purpose**: Plan and agree audit tests and assurance activities to protect operational systems.
**Implementation**:
...
DECLARE
  ...
  v_total_expected INTEGER := 93;
BEGIN
  -- Count how many records were updated
  SELECT COUNT(*) INTO v_updated_count
  FROM requirements_library rl
  JOIN standards_library sl ON rl.standard_id = sl.id
  WHERE sl.name ILIKE '%27002%'
    AND rl.audit_ready_guidance IS NOT NULL
    AND rl.audit_ready_guidance != ''
    AND rl.updated_at > NOW() - INTERVAL '1 minute';
  
  RAISE NOTICE '============================================================================';
  RAISE NOTICE 'UPDATE COMPLETED SUCCESSFULLY';
  RAISE NOTICE '============================================================================';
  RAISE NOTICE 'Requirements updated: % out of % expected', v_updated_count, v_total_expected;
  RAISE NOTICE 'Timestamp: %', NOW();
  
  IF v_updated_count = v_total_expected THEN
    RAISE NOTICE '✅ SUCCESS: All ISO 27002 requirements updated successfully!';
  ELSIF v_updated_count > 0 THEN
    RAISE NOTICE '⚠️  PARTIAL: % requirements updated (expected %)', v_updated_count, v_total_expected;
  ELSE
    RAISE NOTICE '❌ FAILED: No requirements were updated. Check standard existence.';
  END IF;
