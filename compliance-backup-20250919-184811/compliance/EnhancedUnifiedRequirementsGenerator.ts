...
export class EnhancedUnifiedRequirementsGenerator {
  ...
  private selectBestContentSource(req: RequirementDetail): string {
    // FOR UNIFIED REQUIREMENTS: Skip audit_ready_guidance to avoid injecting audit evidence
    // Only use clean content sources: official_description > description
    // (audit_ready_guidance is reserved for unified guidance only)
    const sources = [
      { content: (req as any).official_description, type: 'official_description' },
      { content: req.description, type: 'description' }
      // NOTE: audit_ready_guidance intentionally excluded to prevent audit evidence injection
    ];
