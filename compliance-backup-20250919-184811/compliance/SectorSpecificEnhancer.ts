...
export class SectorSpecificEnhancer {
   ...
   * NOW WITH TIERED REQUIREMENTS: Essential entities get enhanced requirements
   */
  static enhanceSubRequirements(
    ...
    // Get entity classification for tiered requirements
    const entityClassification = NIS2EntityClassificationService.getEntityClassification(sectorId);
    const isEssentialEntity = entityClassification?.entityType === 'essential';

    // Apply comprehensive sector-specific integration by enhancing existing requirements
    enhancedSubReqs = this.integrateNetworkSecurityGuidance(enhancedSubReqs, categoryTitle, enhancement.sectorName, isEssentialEntity);
    enhancedSubReqs = this.integrateIncidentResponseGuidance(enhancedSubReqs, categoryTitle, enhancement.sectorName, isEssentialEntity);
    enhancedSubReqs = this.integrateAccessControlGuidance(enhancedSubReqs, categoryTitle, enhancement.sectorName, isEssentialEntity);
    enhancedSubReqs = this.integrateDataProtectionGuidance(enhancedSubReqs, categoryTitle, enhancement.sectorName, isEssentialEntity);
    enhancedSubReqs = this.integratePhysicalSecurityGuidance(enhancedSubReqs, categoryTitle, enhancement.sectorName, isEssentialEntity);

    // Apply tiered requirements based on entity type
    if (isEssentialEntity) {
      enhancedSubReqs = this.applyEssentialEntityEnhancements(enhancedSubReqs, categoryTitle, entityClassification);
    }
  ...
  /**
   * Apply Essential Entity enhanced requirements
   */
  private static applyEssentialEntityEnhancements(
    subRequirements: string[],
    categoryTitle: string,
    entityClassification: NIS2EntityClassification | null
  ): string[] {
    if (!entityClassification || entityClassification.entityType !== 'essential') {
      return subRequirements;
    }

    return subRequirements.map(req => {
      // Skip if already enhanced to prevent duplicates
      if (req.includes('ESSENTIAL ENTITY') || req.includes('ANNEX I')) {
        return req;
      }

      // Add Essential Entity enhancements based on category
      if (categoryTitle.toLowerCase().includes('incident')) {
        return req + ' - ESSENTIAL ENTITY REQUIREMENT (Annex I): Enhanced incident response with 24/7 monitoring capabilities, mandatory senior management involvement, and stricter coordination with national authorities. Incident response testing must be conducted every 6 months with scenarios specific to critical infrastructure protection.';
      }

      if (categoryTitle.toLowerCase().includes('continuity') || categoryTitle.toLowerCase().includes('backup')) {
        return req + ' - ESSENTIAL ENTITY REQUIREMENT (Annex I): Mandatory business continuity plans with redundant systems, tested every 6 months. Must maintain alternative communication channels and backup facilities capable of supporting critical operations for minimum 72 hours during cyber incidents.';
      }

      if (categoryTitle.toLowerCase().includes('risk')) {
        return req + ' - ESSENTIAL ENTITY REQUIREMENT (Annex I): Enhanced cybersecurity risk management with quarterly board-level reviews, mandatory threat intelligence integration, and comprehensive supply chain risk assessments covering all critical suppliers and service providers.';
      }

      if (categoryTitle.toLowerCase().includes('monitoring') || categoryTitle.toLowerCase().includes('detection')) {
        return req + ' - ESSENTIAL ENTITY REQUIREMENT (Annex I): Continuous 24/7 security monitoring with automated threat detection, real-time incident correlation, and immediate alerting to national cybersecurity authorities for critical incidents affecting essential services.';
      }

      // Default enhancement for other categories
      return req + ' - ESSENTIAL ENTITY REQUIREMENT (Annex I): Enhanced requirements apply due to critical infrastructure status - stricter compliance monitoring, accelerated incident reporting, and mandatory cybersecurity governance oversight.';
    });
  }
  ...
  private static integrateNetworkSecurityGuidance(
    ...
    categoryTitle: string,
    sectorName: string,
    isEssentialEntity: boolean = false
  ): string[] {
