...
export const UnifiedRequirementsTab: React.FC<UnifiedRequirementsTabProps> = ({
  ...
  // Local state for generated content to trigger re-renders
  const [localGeneratedContent, setLocalGeneratedContent] = React.useState<Record<string, any[]>>({});
  const [isGeneratingContent, setIsGeneratingContent] = React.useState(false);
  
  // Auto-generate content when mappings change
  React.useEffect(() => {
    const generateAllContent = async () => {
      if (isGeneratingContent) return; // Prevent duplicate generation
      
      console.log('[AUTO-GENERATE] Starting content generation for', filteredUnifiedMappings?.length || 0, 'categories');
      setIsGeneratingContent(true);
      
      const newContent: Record<string, any[]> = { ...localGeneratedContent };
      
      for (const mapping of filteredUnifiedMappings || []) {
        const categoryName = mapping.category.replace(/^\d+\. /, '');
        
        // Skip if content already exists
        if (newContent[categoryName] && newContent[categoryName].length > 0) {
          console.log('[AUTO-GENERATE] Content already exists for:', categoryName);
          continue;
        }
        
        try {
          console.log('[AUTO-GENERATE] Generating content for:', categoryName);
          const content = await generateDynamicContentForCategory(categoryName);
          if (content && content.length > 0) {
            newContent[categoryName] = content;
            console.log('[AUTO-GENERATE] Generated', content.length, 'sections for', categoryName);
          }
        } catch (error) {
          console.error('[AUTO-GENERATE] Error generating content for', categoryName, ':', error);
        }
      }
    ...
    if ((filteredUnifiedMappings?.length || 0) > 0 && generateDynamicContentForCategory) {
      // Small delay to ensure the component is fully rendered
      const timer = setTimeout(generateAllContent, 200);
      return () => clearTimeout(timer);
    }
  }, [filteredUnifiedMappings, generateDynamicContentForCategory]);
  return (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {(() => {
                      ...
                      const categoryName = mapping.category.replace(/^\d+\. /, '');
                      const dynamicContent = localGeneratedContent[categoryName] || [];
