...
import { CategoryCard } from '@/components/compliance/CategoryCard';
import { UnifiedRequirementsDisplay } from '@/components/compliance/UnifiedRequirementsDisplay';
import { UnifiedGuidanceModal } from '@/components/compliance/UnifiedGuidanceModal';
...
import { 
  UnifiedCategory,
  complianceUnificationService,
  useIndustrySectors 
} from '@/services/compliance/ComplianceUnificationService';
                                          ...
                                          <div key={i} className="space-y-3">
                                            {parts.map((part, partIdx) => {
                                              ...
                                              if (firstLine.match(/^[a-p]\)\s+/)) {
                                                // Simple approach: find title by looking for " - " after the main title words
                                                const dashIndex = firstLine.indexOf(' - ');
                                                
                                                let title, description;
                                                if (dashIndex > 10) {
                                                  title = firstLine.substring(0, dashIndex).trim();
                                                  description = (firstLine.substring(dashIndex + 3) + '\n' + restOfLines).trim();
                                                } else {
                                                  // No dash found, whole first line is title
                                                  title = firstLine.trim();
                                                  description = restOfLines;
                                                }
