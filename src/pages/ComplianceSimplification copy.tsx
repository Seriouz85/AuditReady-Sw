...
export default function ComplianceSimplification() {
  ...
  const generateDynamicContentForCategory = async (categoryName: string): Promise<any[]> => {
    ...
    try {
      ...
      if (templateContent.length > 0) {
        complianceCacheService.set(cacheKey, templateContent, 3600000);
  ...
  const { data: mappingData, isLoading, refetch: refetchComplianceData } = useComplianceMappingData(selectedFrameworks, selectedIndustrySector || undefined);
