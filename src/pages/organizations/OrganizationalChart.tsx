      ...
      .nodeContent((node: OrgNodeExtended) => {
        return `
          <div style="padding: 10px; border-radius: 4px; background-color: white; box-shadow: 0 2px 5px rgba(0,0,0,0.15); width: 100%; height: 100%;">
            <div style="font-weight: bold; font-size: 14px;">${node.data?.name || node.name || 'Unnamed'}</div>
            <div style="font-size: 12px; color: #666;">${node.data?.role || node.role || 'No role'}</div>
            ${(node.data?.email || node.email) ? `<div style="font-size: 11px; margin-top: 5px;">${node.data?.email || node.email}</div>` : ''}
          </div>
        `;
      });
...
type TemplateKey = 'blank' | 'ceo' | 'corporate';
  ...
  const renderChartView = () => (
    <Card className="h-full">
      ...
      <CardContent className="h-[calc(100%-80px)] p-0">
        <OrgChartComponent data={nodes} onNodeClick={(node: OrgNodeExtended) => {
